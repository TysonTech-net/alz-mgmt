...
module "spoke_network_primary" {
  ...
  resource_group_name      = resource.azurerm_resource_group.network_primary.name
  ...
  # vWAN settings
  hub_mode       = "vwan"
  virtual_hub_id = data.azurerm_virtual_hub.primary[0].id

  # Hub VNet settings (Not used in vWAN mode)
  hub_vnet_id             = null
